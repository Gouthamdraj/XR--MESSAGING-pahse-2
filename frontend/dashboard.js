...
// // [ADD] after the batteryState Map
...
// // ------- Group logic you requested -------
// // Group states are applied to *both* chips, so they always match:
// // - both offline                     -> 'available' (green)
// // - one/both online, not paired yet  -> 'connecting' (amber)
// // - both online and paired           -> 'busy' (red)
// function computeGroupState() {
//   const leftOnline = onlineDevices.has(XR_LEFT);
//   const rightOnline = onlineDevices.has(XR_RIGHT);

//   const pairKey = [XR_LEFT, XR_RIGHT].sort().join('|');
//   const paired = activePairs.has(pairKey) && leftOnline && rightOnline;

//   if (!leftOnline && !rightOnline) return 'available';
//   if (paired) return 'busy';
//   return 'connecting';
// }

// // Build the single row with the current state for both chips
// function buildRows() {
//   const groupState = computeGroupState();
...
//     {
//       label: 'Provider 1',
//       left:  { text: `XR VISION : ${XR_LEFT.replace('XR-', '')}`,  state: groupState, battery: batteryPct },
//       scribe: 'Scribe 1',
//       right: { text: `XR DOCK : ${XR_RIGHT.replace('XR-', '')}`, state: groupState }
//     }
//   ];
// }
...
//   const leftChip = chip(left.text, left.state);
//   const rightChip = chip(right.text, right.state);
...
//     <div class="col-span-12 md:col-span-2 text-center text-white/90 text-base md:text-lg">${scribe}</div>

//     <div class="col-span-12 md:col-span-4 flex items-center justify-end gap-3">
//       ${rightChip}
//       <div class="flex gap-2">
//         <button class="icon-btn" title="Edit">${Icon.pen}</button>
...
//   socket = io({ path: '/socket.io', transports: ['websocket'], reconnection: true });
...
//   // NEW — listen for live battery updates
...
//   renderDevices(); // initial (both offline -> green)
//   initSocket();
// });
...
function renderSysPills(xrId) {
  ...
  const hasMem = Number.isFinite(t.memUsedMb) && Number.isFinite(t.memTotalMb) && t.memTotalMb > 0;
  const ramPct = hasMem ? Math.round((t.memUsedMb / t.memTotalMb) * 100) : null;
  const temp = (typeof t.deviceTempC === 'number') ? Math.round(t.deviceTempC * 10) / 10 : null;

  const pills = [];
  if (ramPct !== null) pills.push(`<span class="px-2 py-0.5 rounded bg-white/10 text-white/80 text-[11px]">RAM — ${ramPct}%</span>`);
  if (temp !== null) pills.push(`<span class="px-2 py-0.5 rounded bg-white/10 text-white/80 text-[11px]">TEMP — ${temp}°C</span>`);
