 ...
 * 2) window.__SIGNAL_URL__            (optional injection)
 * 3) localStorage('signal_url')       (remember user choice)
 * 4) location.origin                  (same-origin: http://localhost:3000)
 * 5) fallbackNgrok                    (KEEP your current ngrok)
 */
const __qp = new URLSearchParams(location.search);
const __signalOverride = __qp.get('signal');
const __injected = (typeof window !== 'undefined' && window.__SIGNAL_URL__) || null;
